...
class IonExchangeTransport0DData(InitializationMixin, UnitModelBlockData):
    ...
    **EnergyBalanceType.energyPhase** - energy balances for each phase.}""",
    ...
    def initialize_build(
        ...
        # Now calculate service time from breakthrough volume
        calculate_variable_from_constraint(
            self.service_time,
    ...
    def calculate_performance(self):
        ...
        for ion in self.target_ion_set:
            ...
            if bv_key in results and results[bv_key] is not None:
                ...
                _log.info(f"Updated {ion} breakthrough from {old_value:.1f} to {new_value:.1f} BV")
            else:
        ...
        # CRITICAL: Fix outlet mole fractions after removal rates are applied
        # This prevents MCAS from defaulting to 10,000 mg/L concentrations
        outlet_state = self.control_volume.properties_out[0]
        logger.info("Fixing outlet mole fractions after IX calculations...")
        fix_mole_fractions(outlet_state)
        ...
        if suspicious_count > 0:
            ...
            solver = SolverFactory('ipopt')
            solver.options['tol'] = 1e-8
            outlet_dof = degrees_of_freedom(outlet_state)
            if outlet_dof > 0:
                logger.info(f"Attempting to resolve outlet properties (DOF={outlet_dof})...")
                results = solver.solve(outlet_state, tee=False)
                if results.solver.termination_condition == 'optimal':
                    fix_mole_fractions(outlet_state)
                    logger.info("Successfully resolved outlet properties")
    ...
    def _update_removal_rates(self):
        ...
        # Fix mass transfer term for H2O to 0 (no water transfer)
        if ('Liq', 'H2O') in self.control_volume.mass_transfer_term:
            self.control_volume.mass_transfer_term[t, 'Liq', 'H2O'].fix(0)
