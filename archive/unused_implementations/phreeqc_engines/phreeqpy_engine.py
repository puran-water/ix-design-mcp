...
END
    ...
    def calculate_acid_dose_for_degasser(
        ...
        Args:
            influent_water: Water composition
    ...
    async def simulate_chemical_addition(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Async wrapper for water-chemistry-mcp's simulate_chemical_addition.
        Used by degasser model for acid dosing calculations.
        """
        if not WATER_CHEM_AVAILABLE:
            raise ImportError("water-chemistry-mcp not available for chemical addition simulation")
        
        # Import here to avoid circular imports
        from tools.chemical_addition import simulate_chemical_addition as sim_chem_add
        
        # Run the async function
        result = await sim_chem_add(input_data)
        return result
    
    async def calculate_solution_speciation(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Async wrapper for water-chemistry-mcp's calculate_solution_speciation.
        Used by degasser model for CO2 equilibration calculations.
        """
        if not WATER_CHEM_AVAILABLE:
            raise ImportError("water-chemistry-mcp not available for solution speciation")
        
        # Import here to avoid circular imports
        from tools.solution_speciation import calculate_solution_speciation as calc_spec
        
        # Run the async function
        result = await calc_spec(input_data)
        return result
